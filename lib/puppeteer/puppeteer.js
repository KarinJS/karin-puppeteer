...
import { isImgSuffix } from '../common/common.js'
 ...
 * @property {string} data.screensEval 截图元素
 */
...
class Client {
  ...
  async browserInit () {
    ...
    try {
      ...
      logger.error('[chromium] 启动失败 请尝试 pnpm run init 进行初始化')
      logger.error(error)
      process.exit(1)
    }
  }
  ...
  async screenshot (data, opts = { retry: true }) {
    ...
    try {
      ...
      let { type = 'jpeg', quality = 90, omitBackground = false } = data

      /** 校验type参数默认jpeg */
      if (!isImgSuffix(type)) { type = 'jpeg' }

      // 判断页面是否有元素
      let screensEval = data.screensEval || false
      if (screensEval) {
        try {
          const isExist = await page.$(screensEval)
          if (!isExist) {
            screensEval = false
          }
        } catch (e) {
          logger.debug(`[图片生成][${name}] 页面元素获取失败：${e}`)
          screensEval = false
        }
      }
      if (!screensEval) {
      ...
      /** 获取页面元素 */
      const body = await page.$(screensEval)
      ...
      if (!data.multiPage) {
        ...
        this.shoting.pop()
        if (!this.debug && data.hash) HttpServer.file.delete(data.hash)
