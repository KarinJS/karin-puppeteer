...
class Puppeteer {
  ...
  async screenshot (name, data = {}, opts = { retry: true }) {
    ...
    /** waitUntil参数处理 */
    if (!data.pageGotoParams?.waitUntil) data.pageGotoParams.waitUntil = Config.waitUntil
    ...
    try {
      ...
      if (!data.setViewport) {
        ...
        if (Config.setViewport.useBody) {
          ...
          /** 默认视窗大小 */
          data.setViewport = Config.setViewport.data
        }
      } else {
        let { width, height, deviceScaleFactor } = data.setViewport
        /** body宽高 */
        if (Config.setViewport.useBody) {
          data.setViewport.width = Math.round(width || boundingBox.width)
          data.setViewport.height = Math.round(height || boundingBox.height)
          data.setViewport.deviceScaleFactor = Math.round(deviceScaleFactor || 1)
        } else {
          /** 默认视窗大小 */
          const Cfg = Config.setViewport.data
          data.setViewport.width = Math.round(width || Cfg.width)
          data.setViewport.height = Math.round(height || Cfg.height)
          data.setViewport.deviceScaleFactor = Math.round(deviceScaleFactor || Cfg.deviceScaleFactor)
        }
      }

      await page.setViewport(data.setViewport)
