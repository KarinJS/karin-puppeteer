...
class Client {
  ...
  async screenshot (data, opts = { retry: true }) {
    ...
    try {
      ...
      const { type = 'webp', quality = 90, omitBackground = false } = data

      /** 视窗大小和设备像素比 */
      const setViewport = this.setViewport(data)
  ...
  setViewport (data) {
    ...
    if (data.setViewport) {
      ...
      const { width, height, deviceScaleFactor = 1 } = Config.puppeteer.setViewport
      return {
        width: Math.round(width),
        height: Math.round(height),
        deviceScaleFactor
