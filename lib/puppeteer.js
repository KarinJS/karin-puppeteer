...
class Puppeteer {
  ...
  async restart (normal) {
    if (!this.init) {
      /** 页面创建数量到达上限 重启浏览器 */
      if ((this.createPageCount > Config.createPageCount) && this.shoting.length <= 0) {
        this.init = true
        this.createPageCount = 0
        if (normal) logger.mark(`页面创建数量达到单实例 [${Config.createPageCount}] 上限，1秒后将重启浏览器...`)
        // await sleep(5000)
        await this.close()
        setTimeout(async () => {
  ...
  async screenshot (name, data = {}, opts = { retry : true}) {
    logger.mark(`[图片生成][${name}] 开始生成...${this.init ? 'puppeteer 初始化中...' : ''}`)
